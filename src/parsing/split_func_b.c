...
/*   Updated: 2023/09/17 14:07:13 by kyacini          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_lstcmd(char **div, int *tab, t_commande **c, t_vars *var_env)
{
	...
	while (div[i])
	{
		...
		if (tab[i] == 1)
			ft_lstadd_backcmd(c, ft_lst_newcmd(without_exp, supp_quotes(div[i], var_env, tab[i]), 1));
		else if (tab[i] == 2)
			ft_lstadd_backcmd(c, ft_lst_newcmd(without_exp, supp_quotes(div[i], var_env, tab[i]), 2));
		else if (tab[i] == 3)
			ft_lstadd_backcmd(c, ft_lst_newcmd(without_exp, supp_quotes(div[i], var_env, tab[i]), 3));
		else if (tab[i] == 4)
			ft_lstadd_backcmd(c, ft_lst_newcmd(without_exp, supp_quotes(div[i], var_env, tab[i]), 4));
		else if (tab[i] == 5)
		{
			free(without_exp);
			free(div[i]);
